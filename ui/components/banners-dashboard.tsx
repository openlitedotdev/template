...
export default function BannersDashboard() {
  return (
    <>
      ...
      />
      <ScrollArea className="h-[calc(98vh-200px)] p-2">
        {
      BannersMocks.map((banner, index) => (
        <div key={index} className="p-4 border flex flex-col gap-4 mb-2 rounded-lg shadow-sm relative">
          <Heading
            title={banner.title}
            description={banner.description}
          />
          <Image src={banner.image} width="1000" height="1000" className="rounded-lg" alt={banner.title} />
