        ...
        <div className="flex h-20 justify-between gap-8 items-center">
          <div className="flex items-center font-bold">
            <Link href="https://github.com/open-land/commerce" target="_blank">
              <Image src={logoOpenSS} alt="Open source sinergy" className="size-20 object-contain" />
            </Link>
          </div>
              ...
              {links.map((link, index) => {
                return (
                  <li key={index} className="inline-flex">
                    <Link href={link.path} className="border-transparent text-card- hover:text-card-foreground/90 inline-flex items-center border-b-2 pt-px text-base font-medium">{link.name}</Link>
                  </li>
                )
              })}
            </ul>
